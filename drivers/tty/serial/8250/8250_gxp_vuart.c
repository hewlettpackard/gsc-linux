...
static const struct regmap_config gxp_vuart_regmap_cfg = {
    .reg_bits   = 32,
    .val_bits   = 8,
    .reg_stride = 1,
};
...
static int gxp_vuart_handle_irq(struct uart_port *port)
{
	...
	if (handled == 0)
		pr_info("VUART IRQ NO HANDLED ... VSR = 0x%x\n", reg);
	return 1;
}
...
static int gxp_vuart_probe(struct platform_device *pdev)
{
	...
	vuart->cfgReg = devm_regmap_init_mmio(&pdev->dev, base, &gxp_vuart_regmap_cfg);
