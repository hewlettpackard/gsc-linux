			 ...
			 loff_t *f_pos)
{
	...
	if (count < SHARED_MEM_CHIF_DESCRIPTOR_SIZE) {
		dev_err(drvdata->dev, "read length is too small(%zd < %d)\n", count,
		       SHARED_MEM_CHIF_DESCRIPTOR_SIZE);
		return -EINVAL;
	}
			  ...
			  size_t count, loff_t *f_pos)
{
	...
	uint32_t desc_idx;
	uint32_t desc_len;
	int timeout, ret_val = 0;

	uint8_t *dummy = (uint8_t *)kmalloc(PAGE_SIZE, GFP_KERNEL);
			   ...
			   char *buf)
{
	...
	ret += sprintf(buf + ret, "ccb[24] at %p sizeof(ccb)=%zd\n", p_ccb,
		       sizeof(struct CCB));
...
static int chif_probe(struct platform_device *pdev)
{
	...
	if (drvdata->alloc_buf_dma_addr & 0xFFFFF) {
		...
		drvdata->shmem = (void *) ((unsigned long)drvdata->shmem & ~0xFFFFF);
	}

	dev_info(drvdata->dev, "chif: alloc_buf_dma_addr=%pad\n",
	       &drvdata->alloc_buf_dma_addr);
	dev_info(drvdata->dev, "chif: alloc_buf=%px\n", drvdata->alloc_buf);
	dev_info(drvdata->dev, "chif: shmem virt=%px phys=%pad\n", drvdata->shmem,
	       &drvdata->shmem_phys);
	...
	reg &= 0x7fffffff; //bit31:30 are reserved
