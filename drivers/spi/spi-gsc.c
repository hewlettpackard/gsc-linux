...
static const int freq_divisor[] = {4, 5, 6, 7, 8, 10, 13, 16, 32};
...
static u32 gsc_get_frequency(struct gsc_spi *spifi)
{
	...
	if (value >= NO_FREQ_DIVISORS)
		value = NO_FREQ_DIVISORS - 1;

	freq = 400000000 / freq_divisor[value];
	return freq;
}
...
static int gsc_spifi_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, spidev_node) {
		if (of_property_read_u32(spidev_node, "reg", &chip_index)) {
			dev_warn(dev, "%s: missing 'reg' property, skipping\n",
				 spidev_node->full_name);
			continue;
		}
		if (chip_index >= GSC_SPI0_MAX_CHIPSELECT) {
			dev_warn(dev, "%s: chip select %u exceeds max (%d), skipping\n",
				 spidev_node->full_name, chip_index,
				 GSC_SPI0_MAX_CHIPSELECT);
			continue;
		}
