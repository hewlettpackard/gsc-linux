...
#define EVDISTAT_PORTSUSPEND 0x01000000
#define EVDISTAT_HOSTRESUME 0x02000000

#define EVDIEN			0x0004

#define EVDISTAT_MASK	(EVDISTAT_SETUP_DAT_RCV|EVDISTAT_PORTEN_CHANGE| \
						EVDISTAT_PORTRST|EVDISTAT_PORTSUSPEND|EVDISTAT_HOSTRESUME| \
						EVDISTAT_ALL_EP_INT)
#define EVDIEN_MASK	(EVDISTAT_SETUP_DAT_RCV|EVDISTAT_PORTEN_CHANGE| \
					EVDISTAT_PORTRST|EVDISTAT_PORTSUSPEND|EVDISTAT_HOSTRESUME| \
					EVDISTAT_EP0_INT)

//added for UDC panic fix
#define EHCI_USB_RST_EV              0x80000000 //[23] USB bus RESET event
#define EHCI_USB_HOST_OWMER_EV      0x00200000 //[21] USB host change ownership event
#define EHCI_SETUP_DAT               0x00010000 //[16] EP0 SetupData (8) bytes recvd
#define EVDIEN_INTERRUPT_MASK      (EHCI_USB_RST_EV|EHCI_USB_HOST_OWMER_EV| \
					EHCI_SETUP_DAT|0x00000007)


#define EVDCS			0x0008
#define EVDCS_CONNECT		0x00000001
#define EVDCS_PORTENABLESTS	0x01000000
#define EVDCS_DEVWAKE           0x00000002
...
static int gxp_udc_connect(struct gxp_udc_drvdata *drvdata)
{
	...
	// make sure EVDEPSTS and EDVDISTAT registers is zero (no interrupt appending)
	// find out the number of endpoints configured for each vEHCI device and
	// acccordingly clear the status register of those mapped endpoints
	val = (EVDEPSTS_DONE|EVDEPSTS_SHORT|EVDEPSTS_STALLDET);
	for (j = 0; j < drvdata->fepnum + 1; j++)
		writel(val, drvdata->ep[j].base + EVDEPSTS);
	...
	val |= (EVDISTAT_SETUP_DAT_RCV|EVDISTAT_PORTEN_CHANGE|EVDISTAT_PORTRST |
			 EVDISTAT_PORTSUSPEND | EVDISTAT_HOSTRESUME);
	...
	//added for UDC panic fix
	val = readl(drvdata->base + EVDCS);
	//make sure the port is not in the reset state
	while (val & 0x80000000) {
		pr_debug("udc%d in reset state\n", drvdata->vdevnum);
		//port in reset state, try to clear the reset, wait
		writel(EVDIEN_INTERRUPT_MASK, drvdata->base + EVDISTAT);
		//usleep(10);
		usleep_range(1, 100);
		val = readl(drvdata->base + EVDCS);
		time_out_count++;
		if (time_out_count > 100) {
			//This will cause ehci_connect request to fail
			pr_err("%s:port in reset status for 10ms\n", __func__);
			return 0;
		}
	}
	//make sure the port is not in the suspend state
	val = readl(drvdata->base + EVDCS);
	if (val & 0x02000000) {
		pr_info("udc%d in suspend state,doing DevWake\n", drvdata->vdevnum);
		//port in suspend state, try to do DevWake
		writel(val & ~0x00000002, drvdata->base + EVDCS);
		usleep_range(1, 100);
		writel(val | 0x00000002, drvdata->base + EVDCS);
		usleep_range(1, 100);
	}
...
static int gxp_udc_wakeup(struct usb_gadget *gadget)
{
	struct gxp_udc_drvdata *drvdata;
	...
	if (drvdata != NULL) {
		...
		if (!drvdata->remote_wakeup)
			return -EINVAL;

		spin_lock(&drvdata->lock);
		writel(readl(drvdata->base + EVDCS) | EVDCS_DEVWAKE, drvdata->base + EVDCS);
		spin_unlock(&drvdata->lock);
		pr_info("Wakeup done for vdev%d\n", drvdata->vdevnum);
	}
	return 0;
}
						 ...
						 struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType == USB_RECIP_DEVICE) {
		if (le16_to_cpu(ctrl->wValue) == USB_DEVICE_REMOTE_WAKEUP) {
			pr_info("Set remote_wakeup for vdev%d\n", drvdata->vdevnum);
			drvdata->remote_wakeup = 1;
			gxp_udc_in_0byte_status(&drvdata->ep[0]);
			// Clearing DevWake bit of EVDCS.
			writel(readl(drvdata->base + EVDCS) & ~EVDCS_DEVWAKE,
			       drvdata->base + EVDCS);
			return 1;
		}
	}
				 ...
				 struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType == USB_RECIP_DEVICE) {
		if (le16_to_cpu(ctrl->wValue) == USB_DEVICE_REMOTE_WAKEUP) {
			pr_info("Clear remote_wakeup for vdev%d\n", drvdata->vdevnum);
			drvdata->remote_wakeup = 0;
			gxp_udc_in_0byte_status(&drvdata->ep[0]);
			return 1;
		}
	}
...
static irqreturn_t gxp_udc_irq(int irq, void *_drvdata)
{
	...
	if (evgistat & (1<<drvdata->vdevnum)) {
		...
		while (evdistat & EVDISTAT_MASK) {
			...
			if (evdistat & EVDISTAT_PORTSUSPEND) {
				pr_debug("vdev%d get port suspend int and current gadget state %d\n",
						drvdata->vdevnum, drvdata->gadget.state);
				drvdata->suspended_from = drvdata->gadget.state;
				usb_gadget_set_state(&drvdata->gadget, USB_STATE_SUSPENDED);

				if (drvdata->driver && drvdata->driver->suspend)
					drvdata->driver->suspend(&drvdata->gadget);

				// write 1 to clear
				writel(EVDISTAT_PORTSUSPEND, drvdata->base + EVDISTAT);
			}

			if (evdistat & EVDISTAT_HOSTRESUME) {
				pr_debug("vdev%d get host resume int current gadget state %d and resume from %d\n",
						drvdata->vdevnum, drvdata->gadget.state, drvdata->suspended_from);
				usb_gadget_set_state(&drvdata->gadget, drvdata->suspended_from);

				if (drvdata->driver && drvdata->driver->resume)
					drvdata->driver->resume(&drvdata->gadget);

				// write 1 to clear
				writel(EVDISTAT_HOSTRESUME, drvdata->base + EVDISTAT);
			}
