...
static int crps_probe(struct i2c_client *client)
{
	...
	if (rc > 0 && buf[0] != 0) {
		valid = 1;  // Accept ANY model that has content
		dev_info(dev, "PMBUS Model accepted: %s\n", buf);
	} else {
		return dev_err_probe(dev, -ENODEV, "PMBUS Model invalid or empty\n");
	}
	...
	if (rc)
		return dev_err_probe(dev, rc, "PMBUS Failed to probe\n");
