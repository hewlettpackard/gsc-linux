...
static const struct regmap_config gxp_kcs_regmap_cfg = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
};
...
static int hpe_kcs_probe(struct platform_device *pdev)
{
	...
	priv->kcsCfg = devm_platform_ioremap_resource_byname(pdev, "cfg");
	if (IS_ERR(priv->kcsCfg))
		return PTR_ERR(priv->kcsCfg);

	priv->kcsCfgMap = devm_regmap_init_mmio(&pdev->dev, priv->kcsCfg,
						&gxp_kcs_regmap_cfg);
