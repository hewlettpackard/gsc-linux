...
retry:
	 ...
	 */
//	printk("Message answer len %d\n", ssif_msg_len(&ssif_bmc->response));
//	for ( i = 0 ; i < ssif_msg_len(&ssif_bmc->response) ; i++ )
//		printk("%x ", ssif_bmc->response.payload[i]);
//	printk("\n");
...
//	printk("Sending received data to user\n");
			      ...
			      loff_t *ppos)
{
	...
	if (count > sizeof(struct ssif_msg)) {
		 // printk("gxp: Error count too big: %d\n", count);
		return -EINVAL;
	}
	...
	if (ret) {
		// printk("gxp: error copy from user\n");
		goto out;
	}
	...
	if (count >= ssif_msg_len(&ssif_bmc->response)) {
		...
		// printk("gxp: error on message size\n");
		ret = -EINVAL;
	}
	spin_unlock_irqrestore(&ssif_bmc->lock, flags);
	...
	if (ret)
		goto out;
//	printk("Answer received\n");
	// printk("gxp: netfn=%d lun=%d cmd=%d payload[0]=%x \n",
	// 	  ssif_bmc->response.netfn_lun >> 2, ssif_bmc->response.netfn_lun & 0x3, ssif_bmc->response.cmd, ssif_bmc->response.payload[0] );
...
//	printk("1 process is polling the driver\n");
	...
	if (ssif_bmc->is_singlepart_read) {
		...
		if (ssif_bmc->response.len && ssif_bmc->msg_idx < ssif_bmc->response.len) {
			...
			*val = buf[ssif_bmc->msg_idx];
			...
			ssif_bmc->msg_idx++;
			// vejmarie : we have to wait here for the response to be available
			// if it is not then we screw the answer
...
//	printk("Handle write: %x from %x smbus cmd\n", *val, ssif_bmc->smbus_cmd);
...
//	printk("Handling Complete Write\n");
...
//			printk("Busy\n");
	...
	case I2C_SLAVE_STOP:
		 ...
		 * completely.
		 */
//		printk("I2C Slave Stop Received\n");
		// vejmarie
		// We shall track if we have a pending WRITE Event instead last Event
		if (ssif_bmc->last_event == I2C_SLAVE_WRITE_RECEIVED)
			complete_write_received(ssif_bmc);
//		else
//			printk("PEC Received on non WRITE EVENT %d\n", ssif_bmc->last_event);
		/* Reset message index */
		ssif_bmc->msg_idx = 0;
		break;
