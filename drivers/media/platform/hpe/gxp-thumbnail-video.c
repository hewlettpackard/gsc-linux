...
/* Calculate buffer size based on max resolution and max bytes per pixel (4) */
#define VCD_MAX_BUFFER_CALC(w, h, bpp) ((w) * (h) * (bpp))
#define VCD_MAX_SRC_BUFFER_SIZE		(VCD_MAX_BUFFER_CALC(MAX_WIDTH, MAX_HEIGHT, 4) + sizeof(struct v4l2_rect))
				 ...
				 struct gxp_video_addr *addr, size_t size)
{
	...
	/* Calculate the required size based on current resolution and bpp */
	required_size = VCD_MAX_BUFFER_CALC(gWidth, gHeight, gBpp);
	
	/* Use the smaller of the requested size and our calculated size */
	actual_size = min(size, required_size);
	
	/* Add space for v4l2_rect */
	actual_size += sizeof(struct v4l2_rect);
	
	/* Cap at max buffer size if needed */
	if (actual_size > VCD_MAX_SRC_BUFFER_SIZE)
		actual_size = VCD_MAX_SRC_BUFFER_SIZE;

	dev_dbg(video->dev, "Allocating DMA buffer: %zu bytes (%d x %d x %d)",
		 actual_size, gWidth, gHeight, gBpp);
	
	addr->virt = dma_alloc_coherent(video->dev, actual_size, &addr->dma, GFP_KERNEL);

	if (!addr->virt) {
		dev_err(video->dev, "Failed to allocate DMA buffer of size %zu bytes (%zu pages)",
			actual_size, actual_size >> PAGE_SHIFT);
		return false;
	}

	addr->size = actual_size - sizeof(struct v4l2_rect);
	dev_dbg(video->dev, "DMA buffer allocated successfully at %pad (virtual %p)",
		 &addr->dma, addr->virt);
...
{
	if (!addr->virt || !addr->size)
		return;
	
	size_t alloc_size = addr->size + sizeof(struct v4l2_rect);
	
	dev_dbg(video->dev, "Freeing DMA buffer: %zu bytes at %pad",
		alloc_size, &addr->dma);
	
	dma_free_coherent(video->dev, alloc_size, addr->virt, addr->dma);
	addr->size = 0;
	addr->dma = 0ULL;
	addr->virt = NULL;
}
...
static int gxp_video_start_streaming(struct vb2_queue *q, unsigned int count)
{
    ...
    dev_dbg(video->dev,"<TB_VIDEO> Start Streaming\n");
	video->sequence = 0;
	gxpvd_enable(video);
    // fill all buffers 
...
{
	struct gxp_video *video = vb2_get_drv_priv(q);
	dev_dbg(video->dev, "<TB_VIDEO> Stop Streaming\n");
...
static int gxp_video_start(struct gxp_video *video)
{
	...
	/* Calculate buffer size based on current resolution and bpp */
	buffer_size = VCD_MAX_BUFFER_CALC(gWidth, gHeight, gBpp);
	video->max_buffer_size = min_t(size_t, buffer_size, VCD_MAX_SRC_BUFFER_SIZE);

	dev_dbg(video->dev, "Starting video with resolution %dx%d, bpp %d, buffer size %zu",
		 gWidth, gHeight, gBpp, video->max_buffer_size);

	if (!gxp_video_alloc_buf(video, &video->src, video->max_buffer_size)) {
		dev_err(video->dev, "Failed to allocate DMA memory. Try reducing resolution or bits per pixel.");
		return -ENOMEM;
	}
...
static void gxp_video_stop(struct gxp_video *video)
{
	...
	dev_dbg(video->dev, "<TB_VIDEO> Video Stopped\n");
...
static int gxp_video_open(struct file *file)
{
	...
	if (v4l2_fh_is_singular_file(file)) {
		dev_dbg(video->dev, "<TB_VIDEO> Starting Video\n");
...
static int gxp_video_release(struct file *file)
{
	...
	if (v4l2_fh_is_singular_file(file)){
        dev_dbg(video->dev,"<TB_VIDEO> Stopping Video\n");
...
{
	struct gxp_video *video = video_drvdata(file);
    dev_dbg(video->dev,"<TB_VIDEO> GET DV TIMINGS\n");
...
static int gxp_video_init(struct gxp_video *video)
{
	...
	/* Ensure we have at least 16MB of contiguous memory available */
	dma_set_min_align_mask(dev, 0xfffff);
...
int gxpvd_probe(struct platform_device *pdev)
{
    ...
    of_property_read_u32(pdev->dev.of_node, "bits-per-pixel", &bits_per_pixel);
    switch (bits_per_pixel) {
    case 32:
        gBpp = 4;
        break;
    default:
        // Use 2 bytes per pixel (16-bit) as default to reduce memory usage
        gBpp = 2;
        break;
    }
    ...
    if (ret || gWidth > MAX_WIDTH || gWidth < MIN_WIDTH)
        gWidth = MIN_WIDTH;  // Use minimum resolution to save memory

    ret = of_property_read_u32(pdev->dev.of_node, "height", &gHeight);
    if (ret || gHeight > MAX_HEIGHT || gHeight < MIN_HEIGHT)
        gHeight = MIN_HEIGHT;  // Use minimum resolution to save memory

    dev_info(video->dev, "<TB_VIDEO> Configuration: %d x %d with %d bytes per pixel",
        gWidth, gHeight, gBpp);

    // Calculate expected buffer size for diagnostics
    size_t req_buffer_size = VCD_MAX_BUFFER_CALC(gWidth, gHeight, gBpp);
    dev_info(video->dev, "<TB_VIDEO> Required buffer size: %zu bytes (%zu pages)",
             req_buffer_size, (req_buffer_size + PAGE_SIZE - 1) >> PAGE_SHIFT);
	...
	if (IS_ERR(regs)) {
		dev_err(video->dev, "<TB_VIDEO> io-map memory allocate fail");
		err = PTR_ERR(regs);
		goto out;
	}
	...
	if (IS_ERR(video->vcd_regmap)) {
		dev_err(video->dev, "<TN_VD> Failed to initialize VCD regmap!");
	...
	dev_info(video->dev, "GXP video driver probed successfully");
	return 0;
