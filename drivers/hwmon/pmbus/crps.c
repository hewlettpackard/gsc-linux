...
static int crps_probe(struct i2c_client *client)
{
	...
	printk("Power Supply Model Length = %d\n", rc); // rc = 10
	printk("Power Supply Model buff string = %s\n", buf);
	printk("Power Supply Model buff = %d %d %d %d %d %d %d %d %d %d\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7], buf[8], buf[9]);
	if (rc == 7 && strncmp(buf, "03NK260", 7) == 0) {
		valid = 1;
	} else if (rc == 10) {
		//HPE has 10 characters but they are all zero :(
		valid = 1;
	}
	...
	printk("Power Supply ID Length = %d\n", rc);
	printk("Power Supply ID String = %s\n", buf);
