...
static int umac_setup_phy(struct net_device *ndev)
{
	...
	if (phy_handle) {
		...
		umac->phy_dev = of_phy_connect(ndev, phy_handle, &umac_adjust_link,
					       0, interface);
	...
	if (of_phy_is_fixed_link(phy_handle)) {
		struct device_node *fixed_link_node = of_get_child_by_name(phy_handle,
									   "fixed-link");
