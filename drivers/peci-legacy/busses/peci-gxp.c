...
static int gxp_peci_probe(struct platform_device *pdev)
{
	...
	//Set Timing
	writel(0xC3500064, priv->base + PECITCFG);
	...
	strncpy(priv->adapter->name, pdev->name, sizeof(priv->adapter->name));
...
static void gxp_peci_remove(struct platform_device *pdev)
{
